...
class ApsClient:
    ...
    async def download(self, package_name: str, version: VersionRef, target: Path) -> Path:
        ...
        async with httpx.AsyncClient(follow_redirects=True, timeout=self.settings.aps_download_timeout_seconds) as client:
            await self._download_response(client, url, target, headers=headers, params=params)
        ...
        *,
        ...
        async with client.stream("GET", url, params=params, headers=headers) as response:
            if response.status_code == 202:
                ...
                await self._wait_job(client, data, target, headers)
                return
    ...
    async def _wait_job(self, client: httpx.AsyncClient, data: dict, target: Path, headers: dict[str, str]) -> None:
        import asyncio
        ...
        while True:
            ...
            if body.get("status") == "succeeded":
                ...
                await self._download_response(client, self._absolute_url(file_url), target, headers=headers)
                return
